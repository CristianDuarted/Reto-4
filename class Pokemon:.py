...
class TypeRelations:
    ...
    def get_effectiveness(self, attack_type: str, defender_types : list[str]) -> float:
         ...
         for defender in defender_types:
             if attack_type in self.diccionario:
                 multiplier *= self.diccionario[attack_type][defender]
         return multiplier
    
types = TypeRelations()

print(types.get_effectiveness("Fire", ["Grass"]))          # 2.0
print(types.get_effectiveness("Fire", ["Water"]))          # 0.5
print(types.get_effectiveness("Fire", ["Grass", "Steel"])) # 4.0